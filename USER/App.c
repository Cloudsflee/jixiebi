...
#define BATTERY_MONITOR_SERVO_ID	1U
...
void CheckBatteryVoltage(void)
{
	...
	/* 优先用总线舵机返回的VIN作为母线电压，避免板载分压系数误差导致误报 */
	if(BusServoReadVin(BATTERY_MONITOR_SERVO_ID, &servoVin))
	{
		if(servoVin > 3000 && servoVin < 13000)
		{
			BatteryVoltage = servoVin;
			return;
		}
	}
